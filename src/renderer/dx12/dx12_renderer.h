...
namespace cg::renderer {
    ...
    public:
        void create_heap(ComPtr <ID3D12Device> &device, D3D12_DESCRIPTOR_HEAP_TYPE type, UINT number = 1,
    ...
    protected:
        ...
        void create_resource_on_upload_heap(ComPtr <ID3D12Resource> &resource, UINT size = 0,
        ...
        create_vertex_buffer_view(const ComPtr <ID3D12Resource> &vertex_buffer, UINT vertex_buffer_size);

        static D3D12_INDEX_BUFFER_VIEW
        create_index_buffer_view(const ComPtr <ID3D12Resource> &index_buffer, UINT index_buffer_size);

        void
        create_constant_buffer_view(const ComPtr <ID3D12Resource> &buffer, D3D12_CPU_DESCRIPTOR_HANDLE cpu_handler);
        ...
        static ComPtr <ID3DBlob> compile_shader(const std::filesystem::path &shader_path, const std::string &entrypoint,
        ...
        void create_resource_on_default_heap(ComPtr <ID3D12Resource> &resource, UINT size = 0,
                                             const std::wstring &name = L"",
                                             D3D12_RESOURCE_DESC *resource_descriptor = nullptr);

        void copy_data(const void *buffer_data, UINT buffer_size, ComPtr <ID3D12Resource> &destination_resource,
                       ComPtr <ID3D12Resource> &intermediate_resource, D3D12_RESOURCE_STATES state_after,
                       int row_pitch = 0, int slice_pitch = 0);

        void
        create_shader_resource_view(const ComPtr <ID3D12Resource> &texture, D3D12_CPU_DESCRIPTOR_HANDLE cpu_handler);
