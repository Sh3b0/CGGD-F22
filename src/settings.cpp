...
std::shared_ptr<settings> cg::settings::parse_settings(int argc, char **argv) {
    ...
    add_options("model_path", "Path to OBJ model",
                cxxopts::value<std::filesystem::path>()->default_value("models/cube.obj"));
    add_options("camera_position", "Camera position",
    ...
    add_options("result_path", "Path to resulted image",
